...
class Retriever:
    ...
    def ln_prob(self, params, calculator, measured_depths, measured_errors, low_P=0.1, high_P=2e5, num_P=400):        
        R, T, metallicity, scatt_factor = params
        ...
        wavelengths, calculated_depths = calculator.compute_depths(R, P_profile, T_profile, abundances, scattering_factor=scatt_factor)
    ...
    def run_emcee(self, wavelength_bins, depths, errors, T_guess, R_guess, metallicity_guess, scatt_factor_guess, star_radius, g, guess_frac_range = 0.5, nwalkers=50, nsteps=1000):        
        guess = np.array([R_guess, T_guess, metallicity_guess, scatt_factor_guess])
        ndim = len(guess)
...
retriever.run_emcee(wavelength_bins, depths, errors, 1200, 9.7e7, 1, 1, 8.0e8, 9.311)
