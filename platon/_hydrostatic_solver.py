"""The hydrostatic solver now lives in _forward_model._hydrostatic (JAX).
This module is kept as a thin numpy reference implementation."""
import numpy as np

from .constants import k_B, AMU, M_sun, Teff_sun, G
from .errors import AtmosphereError


def _get_radii(ln_Ps, planet_mass, planet_radius,
               P_profile, T_profile, mu_profile):
    intermediate_mu = np.interp(ln_Ps[1:], np.log(P_profile), mu_profile) / 2 \
        + np.interp(ln_Ps[0:-1], np.log(P_profile), mu_profile) / 2
    intermediate_T = np.interp(ln_Ps[1:], np.log(P_profile), T_profile) / 2 \
        + np.interp(ln_Ps[0:-1], np.log(P_profile), T_profile) / 2
