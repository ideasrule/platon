...
class CombinedRetriever:
    ...
    def _ln_like(self, params, transit_calc, eclipse_calc, fit_info, measured_transit_depths,
        ...
        try:
            if measured_transit_depths is not None:
                transit_wavelengths, calculated_transit_depths1, transit_info_dict = transit_calc.compute_depths(
                    Rs, Mp, Rp, T1, logZ, CO_ratio,
                ...
                transit_wavelengths, calculated_transit_depths2, transit_info_dict = transit_calc.compute_depths(
                    Rs, Mp, Rp, T2, logZ, CO_ratio,
                    scattering_factor=scatt_factor, scattering_slope=scatt_slope,
                    cloudtop_pressure=cloudtop_P, T_star=T_star,
                    T_spot=T_spot, spot_cov_frac=spot_cov_frac,
                    frac_scale_height=frac_scale_height, number_density=number_density,
                    part_size=part_size, ri=ri, P_quench=P_quench, full_output=True)
                calculated_transit_depths = (calculated_transit_depths1 + calculated_transit_depths2) / 2
    ...
    @staticmethod
    def get_default_fit_info(Rs, Mp, Rp, T1=None, T2=None, logZ=0, CO_ratio=0.53,
