from data_loader import read_species_data
import kappa_interpolation
...
#Call transit calculator
depth_calculator = TransitDepthCalculator(6.4e6, 7e8, 9.8, "Opac", "all_species_masses")
index, P, T = np.loadtxt("t_p_800K.dat", unpack=True, skiprows=1)
abundances = get_abundances("EOS/eos_1Xsolar_cond.dat")

transit_depths = depth_calculator.compute_depths(P, T, abundances) * 100

wavelengths, depths = np.loadtxt("ref_spectra.dat", unpack=True, skiprows=2)
plt.plot(wavelengths, depths)
plt.plot(depth_calculator.lambda_grid, transit_depths)
plt.figure()
plt.plot(wavelengths, depths-transit_depths)
plt.show()

#print P, T
#print abundances.shape
#depth_calculator.compute_depths(
