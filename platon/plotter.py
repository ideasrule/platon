...
class Plotter():
    ...
    def plot_retrieval_TP_profiles(self, retrieval_result, plot_samples=False, plot_1sigma_bounds=True, num_samples=100, prefix=None):
        ...
        if retrieval_result.retrieval_type == "dynesty":
            ...
            best_params_arr = retrieval_result.samples[np.argmax(retrieval_result.logp)]

        elif retrieval_result.retrieval_type == "emcee":
            equal_samples = np.copy(retrieval_result.flatchain)
            best_params_arr = retrieval_result.flatchain[np.argmax(retrieval_result.flatlnprobability)]
        ...
        for index in indices:
            ...
            temperature_arr.append(t_p_profile.temperatures)

        plt.figure()
        if plot_samples:
            plt.plot(np.array(temperature_arr).T, t_p_profile.pressures / BAR_TO_PASCALS, color='b', alpha=0.25, zorder=2, label='samples') 
        if plot_1sigma_bounds:
            plt.fill_betweenx(t_p_profile.pressures / BAR_TO_PASCALS, np.percentile(temperature_arr, 16, axis=0),
                            np.percentile(temperature_arr, 84, axis=0), color='0.1', alpha=0.25, zorder=1, label='1$\\sigma$ bounds')  

        params_dict = retrieval_result.fit_info._interpret_param_array(best_params_arr)
        t_p_profile.set_from_params_dict(profile_type, params_dict)
        plt.plot(t_p_profile.temperatures, t_p_profile.pressures / BAR_TO_PASCALS, zorder=3, color='r', label='best fit')

        plt.yscale('log')   
        plt.ylim(min(t_p_profile.pressures / BAR_TO_PASCALS), max(t_p_profile.pressures / BAR_TO_PASCALS))
    ...
    def plot_atm_abundances(self, atm_info, min_abund=1e-9, prefix=None):
        ...
        plt.tight_layout()
        if prefix is not None:
            plt.savefig(prefix + "_atm_abundances.pdf")
